...
query1 = stediScoreStreamingDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value") \
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
    .option("topic", "risk-topic") \
    .option("checkpointLocation", "/tmp/kafka-checkpoint-1") \
    .option("failOnDataLoss", "false") \
    .start()
query2 = stediScoreStreamingDF.writeStream.outputMode("append").format("console").start()
